...
namespace ToFormatSharp.Core
{
    public class LongString
    {
        ...
        public LongString Substring(long start = 0, long length = 0)
        {
            if (start >= chars.LongLength) return "";

            if (length == 0) length = chars.LongLength - start;
            if (length > chars.LongLength) length = chars.LongLength - start;
